...
static struct workqueue_struct *release_filter_wq;
...
#define MAX_INITIAL_SEQ_NUMBER (UINT_MAX)
...
/* Functions for handling the difference of seq numbers between primary and secondary sockets after recovery. 
 * NOTE those function will not work if more than 4GB are sent.
 */

/* Update the client delta (received packet stream) only if it is increasing
 */
...
/* Update the client delta (received packet stream)
 */
void set_idelta_seq_hard(struct net_filter_info* filter, __u32 end_seq){
        filter->idelta_seq= end_seq- filter->in_initial_seq;
}

/*  Update the primary delta (outgoing packet stream) only if it is increasing
 */
void set_odelta_seq(struct net_filter_info* filter, __u32 last_ack){
	if(filter->odelta_seq < last_ack- filter->primary_initial_out_seq)
		filter->odelta_seq= last_ack- filter->primary_initial_out_seq;
}

/* For incoming packets. Compute the correct seq number to feed the socket after recovery.
 */
__u32 get_iseq_in(struct net_filter_info* filter, __u32 seq){
	return seq- filter->idelta_seq;
}

/* For incoming packets. Compute the correct ack to feed the socket after recovery.
 */
__u32 get_oseq_in(struct net_filter_info* filter, __u32 last_ack){
	return last_ack- filter->odelta_seq- filter->primary_initial_out_seq + filter->my_initial_out_seq;
}

/* For outgoing packets. Compute the correct ack to send after recovery.
 */
__u32 get_iseq_out(struct net_filter_info* filter, __u32 ack){
        return ack+ filter->idelta_seq;
}

/* For outgoing packets. Compute the correct seq number to send after recovery.
 */
__u32 get_oseq_out(struct net_filter_info* filter, __u32 seq){
	return seq - filter->my_initial_out_seq + filter->odelta_seq+ filter->primary_initial_out_seq;
}

int ack_is_in_send_buffer_flush_window(struct net_filter_info* filter,  __u32 ack);
...
struct send_buffer{
	...
	__u32 last_ack;
        spinlock_t lock;
	struct rw_semaphore flushing_lock;
	int flushed;
...
void init_send_buffer(struct send_buffer **send_buffer){
		...
		init_rwsem(&se_buffer->flushing_lock);
...
/* Tells you id the send buffer has been flushed over the socket.
 * This can appen only during recovery after failure.
 *
 */
...
int ack_is_in_send_buffer_flush_window(struct net_filter_info* filter,  __u32 ack){                       
        //trace_printk("filter->primary_initial_out_seq %u filter->odelta_seq %u ack %u filter->send_buffer->bytes_to_flush %u\n", filter->primary_initial_out_seq, filter->odelta_seq, ack, filter->send_buffer->bytes_to_flush);
	return ( (filter->primary_initial_out_seq+ filter->odelta_seq < ack) && (ack < filter->primary_initial_out_seq+ filter->odelta_seq + filter->send_buffer->bytes_to_flush));
}
...
void set_pending_send_on_send_buffer(struct send_buffer *send_buffer, int pending_send_syscall){
	send_buffer->pending_send_syscall= pending_send_syscall;
}

int dec_and_check_pending_send_on_send_buffer(struct send_buffer *send_buffer){
	send_buffer->pending_send_syscall--;
	return send_buffer->pending_send_syscall==0;
}
...
void lock_send_buffer_to_exclude_flushing(struct send_buffer *send_buffer){
	down_read(&send_buffer->flushing_lock);
}

void unlock_send_buffer_to_exclude_flushing(struct send_buffer *send_buffer){
	up_read(&send_buffer->flushing_lock);
}

/* Flush of the send buffer and save the total amount of bytes thta are supposed to be flushed:
...
int flush_send_buffer_first_time(struct send_buffer *send_buffer, struct sock* sock){
	...
	list_for_each_safe(item, n, send_buffer_head){
                entry= list_entry(item, struct send_buffer_entry, list_entry);
		send_buffer->bytes_to_flush+= entry->size- entry->to_consume_start;
	}

	trace_printk("send_buffer->bytes_to_flush %u port %i\n",  send_buffer->bytes_to_flush, ntohs(sock->ft_filter->tcp_param.dport));
	send_buffer_head= &send_buffer->send_buffer_head;
...
static void flush_send_buffer_from_work(struct work_struct* work){
	...
	if(!is_send_buffer_flushed(filter->send_buffer)){
		flush_send_buffer(filter->send_buffer, filter->ft_sock, 0);
	}
	unlock_send_buffer_for_flushing(filter->send_buffer);
	
	put_ft_filter(filter);
	kfree(work);
	trace_printk("port %d done\n", ntohs(filter->tcp_param.dport));
}
	...
	list_for_each_safe(item, n, stable_buffer_head){
			...
			if(remove){
				...
				if(last_byte != entry->to_consume_start){
					// Hole!!
					remove= 1;
					printk("%s trimming data from stable buffer\n",__func__);
					list_del(item);
                                	kfree_skb(entry->data);
                                	kmem_cache_free(stable_buffer_entries, entry);
...
int is_stable_buffer_empty(struct stable_buffer *stable_buffer){
	int ret= 1;
	if(stable_buffer){
		if(!(stable_buffer->first_byte_to_consume == 0 || list_empty(&stable_buffer->stable_buffer_head)))
			ret= 0;
	}
	return ret;
}
...
void print_all_filters(void){
	...
	struct list_head *head;
        ...
        if(!list_empty(&filter_list_head)){
			...
			trace_printk("%s filter: %s usage: %u sk->state: %u pendig_pckt %u ft_tcp_closed %d tw %p\n",  (filter->type & FT_FILTER_FAKE)?"fake":"", filter_id, atomic_read(&filter->kref.refcount), filter->ft_sock?filter->ft_sock->sk_state:0, filter->ft_pending_packets, filter->ft_tcp_closed, filter->ft_time_wait);
                	kfree(filter_id);
		}
        }
...
static void send_release_filter_message_to_primary(struct net_filter_info *filter);
static void send_release_filter_message_to_secondary(struct net_filter_info *filter);

static void release_filter_from_wq(struct work_struct* work){
	...
	if (filter){
		...
		if(!(filter->type & FT_FILTER_FAKE)){
			if(ft_is_filter_primary(filter)){
				...
				if(ft_is_filter_secondary(filter)){
					flush_workqueue(pckt_dispatcher_pool[PCKT_DISP_POOL_SIZE]);
					remove_filter(filter);
					if(!filter->ft_req)
						send_release_filter_message_to_primary(filter);
				}
				else{
					remove_filter(filter);
				}
...
static void release_filter(struct kref *kref){
	...
	if (filter){
		...
		if(!(filter->type & FT_FILTER_FAKE) && ft_is_filter_secondary(filter) && !filter->ft_primary_closed){
			filter_printed= print_filter_id(filter);
			printk("ERRROR deleting %s filter %s before primary\n", (filter->type & FT_FILTER_FAKE)?"fake":"", filter_printed);
			if(filter_printed)
				kfree(filter_printed);
		}
		...
		if(work){
			work->filter= filter;
			INIT_WORK( (struct work_struct*)work, release_filter_from_wq);
			queue_work(release_filter_wq, (struct work_struct*)work);
		}
	}
}
...
void get_ft_filter(struct net_filter_info* filter){
	...
	/*if(atomic_read(&filter->kref.refcount)==1)
		printk("%s ERROR\n", __func__);*/
...
static struct net_filter_info* find_filter_count_drop(struct ft_pid *creator, int filter_id, int is_child, __be32 daddr, __be16 dport){
	struct net_filter_info* filter= NULL;
        struct list_head *iter= NULL;
		...
		list_for_each(iter, &filter_list_head) {
			...
			if( are_ft_pid_equals(&objPtr->creator, creator)
				...
				if( !is_child && !(objPtr->type & FT_FILTER_CHILD) ){
					if(atomic_read(&objPtr->kref.refcount)>0)
						printk("ERROR %s ref count filter port %d is %d\n", __func__,ntohs(dport), atomic_read(&objPtr->kref.refcount));
...
static struct net_filter_info* find_filter(struct ft_pid *creator, int filter_id, int is_child, __be32 daddr, __be16 dport){
        struct net_filter_info* filter= NULL;
        struct list_head *iter= NULL;
        struct net_filter_info *objPtr= NULL;
...
static struct net_filter_info* find_and_try_get_filter(struct ft_pid *creator, int filter_id, int is_child, __be32 daddr, __be16 dport, int* got_filter){
...
next:	if(fake_filter){
		if(!(fake_filter->type & FT_FILTER_FAKE)){
			...
			printk("ERROR %s trying to substitute a real filter %s req %p sock %p (sk_state %d)tw %p\n", __func__, pfake, fake_filter->ft_req, fake_filter->ft_sock, (fake_filter->ft_sock)?fake_filter->ft_sock->sk_state:-1, fake_filter->ft_time_wait);
			trace_printk("ERROR %s trying to substitute a real filter %s req %p sock %p (sk_state %d)tw %p\n", __func__, pfake, fake_filter->ft_req, fake_filter->ft_sock, (fake_filter->ft_sock)?fake_filter->ft_sock->sk_state:-1, fake_filter->ft_time_wait);
			kfree(pfake);
			goto out;
		}
		ret= 1;
		...
		filter->primary_initial_out_seq= fake_filter->primary_initial_out_seq;
...
static int init_filter_common(struct net_filter_info* filter, int primary){
	...
	filter->idelta_seq= 0;
        ...
        filter->my_initial_out_seq= 0;
        filter->primary_initial_out_seq= 0;
	filter->in_initial_seq= 0;
...
void ft_grown_mini_filter(struct sock* sk, struct request_sock *req){
	struct net_filter_info* filter= req->ft_filter;
	if(filter){
		get_ft_filter(filter);
		spin_lock_bh(&filter->lock);
		
		filter->my_initial_out_seq= tcp_sk(sk)->snd_nxt;
		filter->in_initial_seq= tcp_sk(sk)->rcv_nxt;
		...
		if(filter->type & FT_FILTER_PRIMARY_REPLICA){
			...
			if(filter->type & FT_FILTER_SECONDARY_REPLICA){
				...
				if(add_filter_coping_pending(filter)){
					printk("ERROR %s a filter was found while creating a FT_FILTER_PRIMARY minifilter from a FT_FILTER_PRIMARY_AFTER_SECONDARY_REPLICA port %d\n", __func__, ntohs(dport));
				}
			}
		}
...
static int get_handshake_param(struct sk_buff *skb, __be32 *source, __be16 *port, __u32 *syn, __u32 *ack, __u32 *rst, int *size, __u32 *seq){
	int ret= -EFAULT;
...
extern int is_switching_device(void);
...
static void dispatch_handshake_msg(struct work_struct* work){
	...
	filter= find_and_try_get_filter(&listen_filter->creator, listen_filter->id, 1 , my_work->source, my_work->port, &got_filter);
	...
	if(filter){
		...
		if(got_filter){
			...
			printk("ERROR %s found a filter with negative count port %d\n", __func__,  ntohs(my_work->port));
			goto out;
		}
	}
...
wait_for_hash:
	if(filter){
		...
		trace_printk("WARNING no filter when injcting ack port %d\n",  htons(my_work->port));
		not_injected= 0;
	}	
...
static int try_complete_handshake_seq_from_hook(struct sk_buff *skb, struct net_filter_info *filter){
	...
	match:		hand_work->ack= skb;
			...
			if(hand_work->syn_seq+1 == hand_work->ack_seq){
				...
				queue_work(pckt_dispatcher_pool[PCKT_DISP_POOL_SIZE], (struct work_struct *) hand_work);
				put_handshake_work(hand_work);
				return NF_STOLEN;
			}
			else{
 ...
 * to the socket.
 */
static int create_handshake_seq(struct rx_copy_work *my_work, struct sk_buff **copy_skb, __be32* copy_source, __be16* copy_port, int* redirect){
	struct rx_copy_msg *msg= (struct rx_copy_msg *) my_work->data;
	struct net_filter_info *filter= ( struct net_filter_info *) my_work->filter, *child_filter;
	struct sk_buff *skb;
	__be32 source=0 ;
	__be16 port= 0;
	__u32 syn= 0, ack= 0, seq= 0, rst=0;
	struct handshake_work *hand_work, *hand_work_app;
	int size=0 , ret= 1, got_filter;
	...
	if(get_handshake_param(skb, &source, &port, &syn, &ack, &rst, &size, &seq)){
		printk("ERROR %s wrong get_handshake_param\n", __func__);
		goto out_no_save;
	}

	if( !(syn||ack) || (syn && ack) || rst){
	//	printk("WARNING %s syn %d ack %d rst %d size %d \n", __func__, syn, ack, rst, size);
		if(syn){
			printk("WARNING %s syn %d ack %d rst %d size %d \n", __func__, syn, ack, rst, size);
			goto out_no_save;
		}
	...
	child_filter= find_and_try_get_filter(&filter->creator, filter->id, 1 , source, port, &got_filter);

        if(child_filter){

                if(got_filter){
                        /*if(!(child_filter->type & FT_FILTER_FAKE)){
                                printk("WARINIG %s found a filter port %d\n", __func__,  ntohs(port));
                        }*/
	...
	if(size>0 && !(size==1 && syn)){


		__skb_pull(skb, ip_hdrlen(skb));
        	skb_reset_transport_header(skb);

        	ret= try_complete_handshake_seq_from_hook(skb, filter);
...
static void dispatch_release_filter_msg (struct work_struct* work){
        struct rx_copy_work *my_work=  (struct rx_copy_work *) work, *next_work= NULL;
...
again:	spin_lock_bh(&filter->lock);
	...
	if(my_work->count==0){
        	my_work->count++;
		if(!my_work->not_queued)
	        	next_work= remove_rx_copy_work(filter, my_work);
		if(next_work){
			printk("WARNING next work port %d primary was closed on port %d\n", ntohs(filter->tcp_param.dport), next_work->primary_was_closed);
			queue_work(filter->rx_copy_wq, (struct work_struct*) next_work);
		}
        }
...
out:	pcn_kmsg_free_msg(msg);
	kmem_cache_free(rx_copy_work_entries, my_work);
...
static void dispatch_copy_msg_for_listen(struct work_struct* work){
	...
	//try collect handshake packets
        if(create_handshake_seq(my_work, &skb, &source, &port, &redirect)){
		 put_ft_filter(filter);
...
again:
	...
	if(child_filter->ft_primary_closed==1){
        	printk("%s WARNING primary closed is set on port %d\n", __func__, ntohs(child_filter->tcp_param.dport));
        }
        ...
        if(child_filter->type & FT_FILTER_ENABLE){
        	//trace_printk("REDIRECTING msg id %d to port %d\n", msg->pckt_id, ntohs(child_filter->tcp_param.dport));
		rx_copy_wq= child_filter->rx_copy_wq;
                child_filter->ft_pending_packets++;
		my_work->filter= child_filter;
                my_work->count= -600;
		my_work->not_queued= 1;
                //work->time= time;
...
static void dispatch_copy_msg(struct work_struct* work){
	...
	if(my_work->primary_was_closed==2){
		 queue_work(pckt_dispatcher_pool[PCKT_DISP_POOL_SIZE+1], work);
		 return;
	}
...
again:  filter= find_and_get_filter(&msg->creator, msg->filter_id, msg->is_child, msg->daddr, msg->dport);
		...
		if(msg->pckt_id==1){
			...
			printk("ERROR: %s no filter for pckt id %lld port %d\n", __func__, msg->pckt_id, ntohs(msg->dport));
			pcn_kmsg_free_msg(msg);
		}
        }
...
static int handle_release_filter_primary(struct pcn_kmsg_message* inc_msg){
	struct release_filter_msg *msg= (struct release_filter_msg* ) inc_msg;
	struct net_filter_info *filter;

	filter= find_filter_count_drop(&msg->creator, msg->filter_id, msg->is_child, msg->daddr, msg->dport);
        if(filter){

		spin_lock_bh(&filter->lock);
		if(!filter->waiting){
			printk("ERROR secondary is releasing filter port %d before primary\n",  ntohs(msg->dport));
		}
		else{
			filter->ft_primary_closed++;
			wake_up_process(filter->waiting);
		}
		spin_unlock_bh(&filter->lock);
	}
	else{
		printk("ERROR %s filter port %d not found\n", __func__,  ntohs(msg->dport));
	}
...
static int handle_release_filter(struct pcn_kmsg_message* inc_msg){
	...
	filter= find_and_get_filter(&msg->creator, msg->filter_id, msg->is_child, msg->daddr, msg->dport);
		...
		printk("ERROR %s no filter for port %d", __func__, ntohs(msg->dport));
		pcn_kmsg_free_msg(msg);
	}
out:
	return ret;
...
static int create_release_filter_msg(struct net_filter_info *filter, int primary, struct release_filter_msg **msg, int *msg_size){
	...
	if(primary==0)
		...
		message->header.type= PCN_KMSG_TYPE_FT_RELEASE_FILTER_PRIMARY;
	message->header.prio= PCN_KMSG_PRIO_NORMAL;
...
static void send_release_filter_message_to_secondary(struct net_filter_info *filter){
	struct release_filter_msg* msg;
        ...
        ret= create_release_filter_msg(filter, 0, &msg, &msg_size);
        if(ret)
                return;
...
static void send_release_filter_message_to_primary(struct net_filter_info *filter){
        ...
        ret= create_release_filter_msg(filter, 1, &msg, &msg_size);
        if(ret)
                return;

        send_to_primary(filter->ft_popcorn, (struct pcn_kmsg_long_message*) msg, msg_size);
...
static unsigned int check_if_pckt_to_drop(struct net_filter_info *filter, struct sk_buff *skb){
	...
	if(filter->ft_sock && filter->ft_sock->sk_state==TCP_LISTEN){
		if(tcp_header->syn){
                	seq= ntohl(tcp_header->seq);
                	if(seq>MAX_INITIAL_SEQ_NUMBER || filter->ft_tcp_closed==1){
                        	ret= NF_DROP;
		...
		child_filter= find_and_try_get_filter(&filter->creator, filter->id, 1 , ip_hdr(skb)->saddr, tcp_header->source, &got_filter);
			...
			if(got_filter){
				child_sk= child_filter->ft_sock;
				if(child_sk && (child_sk->sk_state==TCP_CLOSE || child_sk->sk_state==TCP_TIME_WAIT
					|| child_sk->sk_state==TCP_FIN_WAIT2 || child_sk->sk_state==TCP_CLOSING || child_sk->sk_state==TCP_LAST_ACK) //this cases the child filter should receive the packt not the listening
					){
					ret= NF_DROP;
				}
				else{
					if(child_filter->ft_time_wait)
						ret= NF_DROP;
				}
				//tcp_v4_send_reset(filter->ft_sock, skb);
				put_ft_filter(child_filter);
...
static unsigned int ft_hook_before_network_layer_primary(struct net_filter_info *filter, struct sk_buff *skb){
	...
	if(ret == NF_ACCEPT && is_there_any_secondary_replica(filter->ft_popcorn)){
                //ft_start_time(&time);
                //send_skb_copy(filter, pckt_id, local_tx, skb);
                sent= try_send_skb_copy(filter, pckt_id, local_tx, skb, 140000);
                if(IS_ERR_VALUE(sent)){
                        ret= NF_DROP;
                        atomic_inc(&primary_dropped);
                        //trace_printk("dropping pckt id %d port %d\n", pckt_id, ntohl(filter->tcp_param.dport));
                }
                else{
                        ret= NF_ACCEPT;
                }
...
static unsigned int ft_hook_before_network_layer_secondary(struct net_filter_info *filter, struct sk_buff* skb, const struct net_device *in){
        long long pckt_id;
	...
	if(in!=dev_get_by_name(&init_net, DUMMY_DRIVER)){
		printk("%s droppint packet\n", __func__);
		return NF_DROP;
	}
	...
	if(iph->protocol == IPPROTO_UDP
			...
			if(sk){
				...
				if(filter){
					...
					if(filter->type & FT_FILTER_SECONDARY_REPLICA){
                				ret= ft_hook_before_network_layer_secondary(filter, skb, in);
        				}
					else{
        					if(filter->type & FT_FILTER_PRIMARY_REPLICA || filter->type & FT_FILTER_PRIMARY_AFTER_SECONDARY_REPLICA){
...
unsigned int ft_hook_before_tcp_primary_after_secondary(struct sk_buff *skb, struct net_filter_info *filter){
	...
	case TCP_ESTABLISHED:
		 ...
		 * update it to now, when switching to use the real socket, if somenthig pending (not acked by the client)
		 * needs to be flushed.
		 */
                if(!is_send_buffer_flushed(filter->send_buffer)){
			...
			if(!is_send_buffer_flushing(filter->send_buffer)){
				...
				//schedule work to try finish flush
	...
	case TCP_LISTEN:
		...
		/*check that I am not opening a pending connection from the worker thread in parallel*/
		hand_work= get_handshake_work(iph->saddr, tcp_header->source);
		ret= NF_ACCEPT;
        	if(hand_work){
                	spin_lock_bh(&hand_work->lock);
		...
		if(req){
			...
			if( (size-tcp_header->syn) > 0){
				...
				//trace_printk("WARNING %s received a unexpected packet during handshake, dropping it.\n", __func__);
				goto out;
			}
			...
			if(!tcp_header->syn && tcp_header->ack){	
				...
				req->ft_filter->primary_initial_out_seq= ntohl(tcp_header->ack_seq);
	...
	case TCP_LAST_ACK:
		...
		*/
		...
		if(!is_send_buffer_flushed(filter->send_buffer)){
			trace_printk("send buffer to flush!!! status %d port %d \n", filter->ft_sock?filter->ft_sock->sk_state:-1, ntohs(tcp_header->source));
		}
...
unsigned int ft_hook_before_tcp_secondary(struct sk_buff *skb, struct net_filter_info *filter){
	...
	case TCP_LISTEN:
		...
		//trace_printk("letting pckt transiting on status %d: syn %u ack %u fin %u seq %u end seq %u size %u ack_seq %u port %i\n", filter->ft_sock->sk_state, tcp_header->syn, tcp_header->ack, tcp_header->fin, start, end, size,ntohl( tcp_header->ack_seq), ntohs(tcp_header->source));
...
int flush_send_buffer_in_filters(void){
        struct list_head *iter= NULL;
        struct net_filter_info *filter= NULL;
        int ret= 0, pending_send_syscall;
	char* key;
...
again:
				...
				if(pending_send_syscall==0){
					if(filter->type & FT_FILTER_ENABLE && filter->ft_sock && filter->ft_sock->sk_state!=TCP_CLOSE
						...
						ret= flush_send_buffer_first_time(filter->send_buffer, filter->ft_sock);
						if(ret){
							unlock_send_buffer_for_flushing(filter->send_buffer);
							goto out;
						}
...
int flush_pending_pckt_in_filters(void){
	...
	for(i=0; i< PCKT_DISP_POOL_SIZE+2; i++){
...
int update_filter_type_after_failure(void){
	...
	struct list_head *iter= NULL, *n=NULL;
        struct net_filter_info *filter= NULL;
        int ret= 0;
	struct list_head head_filter_to_put;
	struct list_filters* new;

	INIT_LIST_HEAD(&head_filter_to_put);
...
static int __init ft_filter_init(void){
	...
	release_filter_wq= alloc_workqueue("release_filter_wq", WQ_MEM_RECLAIM , 0);
	...
	pcn_kmsg_register_callback(PCN_KMSG_TYPE_FT_RELEASE_FILTER_PRIMARY, handle_release_filter_primary);
